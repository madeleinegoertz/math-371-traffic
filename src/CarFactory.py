...
class CarFactory():
    ...
    '''
    def __init__(self, auto_prob, car_prob, p_factor, t_reaction, db_val):
        ...
        self.lane_change_human_car = LaneChange(p = p_factor,
                                                db = db_val,
                                                gap_min = MAIN_SMIN,
                                                bsave = MAIN_BSAVE_CAR,
                                                bias_right = BIAS_RIGHT_CAR)

        self.lane_change_human_truck = LaneChange(p = p_factor,
                                                  db = db_val,
                                                  gap_min = MAIN_SMIN,
                                                  bsave = MAIN_BSAVE_TRUCK,
                                                  bias_right = BIAS_RIGHT_TRUCK)

        self.lane_change_auto_car = LaneChange(p = p_factor,
                                                   db = db_val ,
                                                   gap_min = MAIN_SMIN,
                                                   bsave = MAIN_BSAVE_CAR,
                                                   bias_right = BIAS_RIGHT_AUTO)

        self.lane_change_auto_truck = LaneChange(  p=p_factor,
                                                   db=db_val ,
                                                   gap_min=MAIN_SMIN,
                                                   bsave=MAIN_BSAVE_TRUCK,
                                                   bias_right=BIAS_RIGHT_AUTO)
